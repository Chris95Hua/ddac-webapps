...
--
-- Host: 127.0.0.1
-- Generation Time: Sep 21, 2017 at 01:13 PM
...
BEGIN
	IF EXISTS(SELECT page_id FROM page WHERE page_id = _page_id) THEN
    	IF EXISTS(SELECT page.page_id FROM page LEFT JOIN page_permission ON page.page_id = page_permission.page_id WHERE page.page_id = _page_id AND page_permission.role_id = _role_id) THEN
        	BEGIN
        		SELECT * FROM page WHERE page_id = _page_id LIMIT 1;
            END;
        ELSE
        	SELECT -1 AS page_id;
        END IF;
...
CREATE DEFINER=`dev`@`localhost` PROCEDURE `get_page_by_slug` (IN `_slug` VARCHAR(32), IN `_role_id` TINYINT)  BEGIN
	IF EXISTS(SELECT page_id FROM page WHERE page_slug = _slug) THEN
    	IF EXISTS(SELECT page.page_id FROM page LEFT JOIN page_permission ON page.page_id = page_permission.page_id WHERE page.page_slug = _slug AND page_permission.role_id = _role_id) THEN
        	BEGIN
        		SELECT * FROM page WHERE page_slug = _slug LIMIT 1;
            END;
        ELSE
        	SELECT -1 AS page_id;
        END IF;
...
INSERT INTO `airport` (`iata_code`, `description`, `city`, `country`) VALUES
...
INSERT INTO `currency` (`currency_id`, `code`, `rate`) VALUES
(1, 'USD', 1),
(2, 'RUB', 58),
(3, 'MYR', 4);
...
INSERT INTO `user` (`user_id`, `full_name`, `birth_date`, `email`, `password`, `salt`, `date_joined`, `role_id`) VALUES
(1, 'admin', '0000-00-00', 'admin@uia.com', '541048b104fec9c4f0e4b17d379ccb17b06b3797001feaf2cbf8241e520f4134', 0xeeec663e535b36beb62fe9dcc98cff982777d9f730ab57ea7413b9ad22491ca9, '2017-09-10', 1);
...
ALTER TABLE `page_permission`
  ...
  ADD KEY `page_id` (`page_id`,`role_id`),
...
--
ALTER TABLE `page_permission`
  MODIFY `permission_id` int(10) UNSIGNED NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=22;
--
...
--
-- Constraints for table `page_permission`
--
ALTER TABLE `page_permission`
  ADD CONSTRAINT `page_permission_ibfk_1` FOREIGN KEY (`page_id`) REFERENCES `page` (`page_id`) ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `page_permission_ibfk_2` FOREIGN KEY (`role_id`) REFERENCES `user_role` (`role_id`) ON DELETE CASCADE ON UPDATE CASCADE;

--
-- Constraints for table `route`
--
ALTER TABLE `route`
  ADD CONSTRAINT `route_ibfk_1` FOREIGN KEY (`source`) REFERENCES `airport` (`iata_code`) ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `route_ibfk_2` FOREIGN KEY (`destination`) REFERENCES `airport` (`iata_code`) ON DELETE CASCADE ON UPDATE CASCADE;

--
-- Constraints for table `user`
--
ALTER TABLE `user`
  ADD CONSTRAINT `user_ibfk_1` FOREIGN KEY (`role_id`) REFERENCES `user_role` (`role_id`) ON DELETE NO ACTION ON UPDATE CASCADE;
